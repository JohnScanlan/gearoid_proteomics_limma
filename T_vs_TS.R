...
expr <- read_excel("C:/Users/crtuser/OneDrive - TCDUD.onmicrosoft.com/Documents/PhD/Project/Data/Gearoid_Proteomics/T vs TS.xlsx", 
                   sheet = "Low exp removed")
...
# Save to file
#write.csv(results, "limma_paired_results_T_vs_TS.csv", row.names = TRUE)

# --- Optional: Plot one protein of interest ---
protein <- "GZMH"  # replace with a protein in your data
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = Group, label = Sample)) +
  ...
  labs(title = "T vs TS", x = "PC1", y = "PC3") +
  theme_minimal() +
  theme(
